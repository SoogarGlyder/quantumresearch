...
// 🚀 PERBAIKAN: Menambahkan '.module' agar Turbopack/Vercel bisa membaca CSS Modules
      ...
      <table className={styles.tabelStyle}>
        ...
        <tbody>
          {dataAbsenStaf.length === 0 ? (
            <tr>
              <td colSpan="5" className={styles.selKosong}>
                Belum ada riwayat absensi staf.
              </td>
            </tr>
          ) : (
            dataAbsenStaf.map((absen) => (
              <tr key={absen._id}>
                ...
                <td style={{ color: "#15803d", fontWeight: "900", fontSize: "16px" }}>
                  <FaClock size={14} />{" "}
                  {new Date(absen.waktuMasuk).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}
                </td>
                <td
                  style={{
                    color: absen.waktuKeluar ? "#ef4444" : "#9ca3af",
                    fontWeight: "900",
                    fontSize: "16px",
                  }}
                >
                  {absen.waktuKeluar ? (
                    <>
                      ...
                      {new Date(absen.waktuKeluar).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}
                    </>
                  ) : (
                    "--:--"
                  )}
