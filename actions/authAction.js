...
import { guardAdmin, ambilSesiAktif } from "../utils/guardHelper";
import {
  PERAN,
  STATUS_USER,
  KONFIGURASI_SISTEM,
...
// KONSTANTA
// ============================================================================
const MAKS_BULK_SISWA = 200;

// ============================================================================
// 1. INTERNAL HELPERS
// ============================================================================
/**
 * @param {string} nomorPeserta
 * @returns {string}
 */
...
export async function prosesLogin(dataFormulir) {
  try {
    ...
    const idInputRaw   = validationHelper.trimInput(
    ...
    const passwordInput = (
      dataFormulir.password || dataFormulir.kataSandi || dataFormulir.pass || ""
    ).toString().trim();
    ...
    if (!sesiBerhasil) {
      return responseHelper.error("Terjadi gangguan server saat membuat sesi. Coba lagi.");
    }

    return responseHelper.success("Login Berhasil! Memuat portal...");
  } catch (error) {
    console.error("[CRITICAL ERROR prosesLogin]:", error);
...
export async function prosesTambahSiswa(dataFormulir) {
  try {
    await connectToDatabase();
    if (!(await guardAdmin())) {
      return responseHelper.error("Akses Ditolak: Khusus Admin & Staff Akademik.");
    }
    ...
    const nomorPeserta = validationHelper.trimInput(dataFormulir.nomorPeserta);
    ...
    const cekDuplikat = await User.exists({
      $or: [{ username }, { nomorPeserta }],
    });
    if (cekDuplikat) {
      return responseHelper.error("Username atau Nomor Peserta sudah digunakan!", null, "DUPLICATE");
    }

    const passwordHashed = await authHelper.buatHash(passPolos);
    const kodeCabangSiswa =
      sesi.kodeCabang === CABANG_QUANTUM.PUSAT.id
        ? ekstrakKodeCabang(nomorPeserta)
        : sesi.kodeCabang;
    ...
    return responseHelper.success(`Siswa didaftarkan! Password awal: ${passPolos}`);
  } catch (error) {
...
export async function prosesTambahPengajar(dataFormulir) {
  try {
    ...
    const duplikat = await User.exists({
      $or: [{ username }, { kodePengajar }],
    });
    if (duplikat) {
      return responseHelper.error(
        "Username atau Kode Pengajar sudah digunakan. Gunakan kode/username yang berbeda atau edit akun yang sudah ada.",
        null,
        "DUPLICATE"
      );
    }

    const kodeCabangPengajar =
      sesi.kodeCabang === CABANG_QUANTUM.PUSAT.id
        ? ekstrakKodeCabang(nomorPeserta)
        : sesi.kodeCabang;
...
export async function prosesBulkTambahSiswa(daftarSiswaRaw) {
  try {
    ...
    if (daftarSiswaRaw.length > MAKS_BULK_SISWA) {
      return responseHelper.error(
        `Maksimum ${MAKS_BULK_SISWA} siswa per satu kali upload. ` +
        `Anda mencoba upload ${daftarSiswaRaw.length} siswa. Pecah menjadi beberapa batch.`,
        null,
        "PAYLOAD_TOO_LARGE"
      );
    }
    ...
    const barisUsername = daftarSiswaRaw.map((s) =>
      validationHelper.trimInput(s.username || s.nomorPeserta).toLowerCase()
    );
    const barisNomor = daftarSiswaRaw.map((s) =>
      validationHelper.trimInput(s.nomorPeserta)
    );
    ...
    return responseHelper.success(
      `${dataSiap.length} dari ${daftarSiswaRaw.length} siswa berhasil didaftarkan.` +
      (dataSiap.length < daftarSiswaRaw.length
        ? ` (${daftarSiswaRaw.length - dataSiap.length} dilewati karena duplikat)`
        : "")
    );
  } catch (error) {
...
// ============================================================================
// 4. SESSION HELPER (dipakai middleware / layout)
// ============================================================================
