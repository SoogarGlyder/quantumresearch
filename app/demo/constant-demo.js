...
export function dapatkanDataDemoSiswa() {
  const now = new Date();
  const formatTgl = (d) => {
    return d.toLocaleDateString("id-ID", { timeZone: "Asia/Jakarta", year: "numeric", month: "2-digit", day: "2-digit" }).split('/').reverse().join('-');
  };

  const tglHariIni = formatTgl(now);
  const tglBesok   = formatTgl(new Date(now.getTime() + 24 * 60 * 60 * 1000));
  const tglKemarin = formatTgl(new Date(now.getTime() - 24 * 60 * 60 * 1000));
  const tglLusaKemarin = formatTgl(new Date(now.getTime() - 48 * 60 * 60 * 1000));
  ...
  const mockSiswa = {
    ...
    koleksiLencana: [
      { idLencana: "first_blood", tanggalDidapat: new Date(now.getTime() - 15 * 24 * 60 * 60 * 1000).toISOString() },
      { idLencana: "burung_hantu", tanggalDidapat: new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000).toISOString() },
      { idLencana: "master_mtk", tanggalDidapat: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString() },
      { idLencana: "konsisten_30", tanggalDidapat: new Date(now.getTime() - 3 * 24 * 60 * 60 * 1000).toISOString() },
  ...
  // =======================================================================
  // 📅 JADWAL HARI INI (Sesuai Skema Asli JSON)
  // =======================================================================
  const mockJadwal = [
    {
      _id: "jdw-demo-1", 
  ...
  // =======================================================================
  // 🚀 GENERATOR JADWAL MASA DEPAN (HANYA SENIN, RABU, JUMAT)
  // =======================================================================
  let countFuture = 1;
  for (let i = 1; i <= 30; i++) {
  ...
  // =======================================================================
  // 🕰️ GENERATOR RIWAYAT MASA LALU (HANYA SENIN, RABU, JUMAT)
  // =======================================================================
  let countPast = 1;
  for (let i = 1; i <= 60; i++) {
    const t = new Date(now.getTime() - i * 24 * 60 * 60 * 1000);
  ...
  const mockLatihan = [
    {
      ...
      link: "https://drive.google.com/file/d/1fajvg9MJ7xbNAfjcwQLEsSlZ5vmkbHyz/preview", url: "https://drive.google.com/file/d/1fajvg9MJ7xbNAfjcwQLEsSlZ5vmkbHyz/preview"
    },
    {
      ...
      link: "https://drive.google.com/file/d/1UadHs2AON-_G5qBgpiaJN972-DRAEOee/preview", url: "https://drive.google.com/file/d/1UadHs2AON-_G5qBgpiaJN972-DRAEOee/preview"
    },
    {
      ...
      link: "https://drive.google.com/file/d/1y0oPJm3EHTS8ofebZQzCJkfxulU6QXZZ/preview", url: "https://drive.google.com/file/d/1y0oPJm3EHTS8ofebZQzCJkfxulU6QXZZ/preview"
    }
  ];
  ...
  const mockKuis = {
    ...
    soal: [
      { _id: "s-1", tipeSoal: "PG", pertanyaan: "Dua roda gigi terhubung. Roda A (12 gigi) dan roda B (24 gigi). Jika roda A diputar 4 kali, roda B berputar...", opsi: [{ label: "A", teks: "1 kali" }, { label: "B", teks: "2 kali" }, { label: "C", teks: "4 kali" }, { label: "D", teks: "8 kali" }], kunciJawaban: "B", bobotExp: 20 },
      { _id: "s-2", tipeSoal: "PG_KOMPLEKS", pertanyaan: "Di antara nama-nama berikut, siapakah yang dikenal sebagai perintis *Ilmu Komputer*? (Pilih lebih dari 1)", opsi: [{ label: "A", teks: "Ada Lovelace" }, { label: "B", teks: "Albert Einstein" }, { label: "C", teks: "Alan Turing" }, { label: "D", teks: "Isaac Newton" }], kunciJawaban: ["A", "C"], bobotExp: 30 },
      { _id: "s-3", tipeSoal: "ISIAN", pertanyaan: "Berapakah hasil perhitungan dari limit berikut: $\\lim_{x \\to 2} \\frac{x^2 - 4}{x - 2}$ ?", kunciJawaban: "4", bobotExp: 50 }
    ]
  };
